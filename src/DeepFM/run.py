...
def main(args):
    ...
    data_path = os.path.join(config_dir, '..', args.dataset.data_path)
    ...
    embedding_dim = args.model_args.embedding_dim
    mlp_dims = args.model_args.mlp_dims
    drop_rate = args.model_args.drop_rate
    ...
    dataset = PairwiseRecommendationDataset(
        ...
        num_negatives=args.model_args.num_negatives
    )
    train_loader = DataLoader(dataset, batch_size=args.model_args.batch_size, shuffle=True)

    # 모델 학습
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.model_args.learning_rate)
    train_pairwise(model, train_loader, optimizer, epochs=args.model_args.epochs)
